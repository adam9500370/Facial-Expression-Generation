...
class Solver(object):
    ...
    def __init__(self, celebA_loader, rafd_loader, fer2013_loader, ferg_db_loader, config):
        # Data loader
        self.celebA_loader = celebA_loader
        self.rafd_loader = rafd_loader
        self.fer2013_loader = fer2013_loader
        self.ferg_db_loader = ferg_db_loader
    ...
    def build_model(self):
        ...
        elif self.dataset == 'BothFer':
            self.G = Generator(self.g_conv_dim, self.c_dim+2, self.g_repeat_num)   # 2 for mask vector
            ...
            for i, (real_x, real_label) in enumerate(self.data_loader):
                ...
                if (i+1) % self.log_step == 0:
                    ...
                    if self.dataset == 'CelebA':
                        ...
                        print('Classification Acc (7 emotional expressions): ', end='')
                    else:
    ...
    def train_multi(self):
        ...
        In the code below, 1 is related to fer2013 and 2 is releated to ferg_db. (the same labels)
        """

        data_loader1 = self.fer2013_loader
        data_loader2 = self.ferg_db_loader
        ...
        for i in range(self.c_dim):
            fixed_c = self.one_hot(torch.ones(fixed_x.size(0)) * i, self.c_dim)
            fixed_c1_list.append(self.to_var(fixed_c, volatile=True))
        ...
        #fixed_zero1 = self.to_var(torch.zeros(fixed_x.size(0), self.c2_dim))     # zero vector when training with fer2013
        fixed_mask1 = self.to_var(self.one_hot(torch.zeros(fixed_x.size(0)), 2)) # mask vector: [1, 0]
        #fixed_zero2 = self.to_var(torch.zeros(fixed_x.size(0), self.c_dim))      # zero vector when training with ferg_db
        ...
        for i in range(start, self.num_iters):
            ...
            real_c1 = self.one_hot(real_label1, self.c_dim)
            fake_c1 = self.one_hot(fake_label1, self.c_dim)
            ...
            # Real images (fer2013)
            out_real, out_cls, _ = self.D(real_x1)
            out_cls1 = out_cls[:, :self.c_dim]      # fer2013 part
            d_loss_real = - torch.mean(out_real)
            d_loss_cls = F.cross_entropy(out_cls1, real_label1)

            # Real images (ferg_db)
            out_real, out_cls, _ = self.D(real_x2)
            out_cls2 = out_cls[:, :self.c_dim]      # ferg_db part
            d_loss_real += - torch.mean(out_real)
            d_loss_cls += F.cross_entropy(out_cls2, real_label2)

            # Compute classification accuracy of the discriminator
            if (i+1) % self.log_step == 0:
                accuracies = self.compute_accuracy(out_cls1, real_label1, 'fer2013')
                log = ["{:.2f}".format(acc) for acc in accuracies.data.cpu().numpy()]
                print('Classification Acc (fer2013): ', end='')
                print(log)
                accuracies = self.compute_accuracy(out_cls2, real_label2, 'ferg_db')
                log = ["{:.2f}".format(acc) for acc in accuracies.data.cpu().numpy()]
                print('Classification Acc (ferg_db): ', end='')
                print(log)

            # Fake images (fer2013)
            #fake_c = torch.cat([fake_c1, zero1, mask1], dim=1)
            fake_c = torch.cat([fake_c1, mask1], dim=1)
            fake_x1 = self.G(real_x1, fake_c)
            fake_x1 = Variable(fake_x1.data)
            out_fake, _, _ = self.D(fake_x1)
            d_loss_fake = torch.mean(out_fake)

            # Fake images (ferg_db)
            #fake_c = torch.cat([zero2, fake_c2, mask2], dim=1)
            fake_c = torch.cat([fake_c2, mask2], dim=1)
            fake_x2 = self.G(real_x2, fake_c)
            out_fake, _, _ = self.D(fake_x2)
            d_loss_fake += torch.mean(out_fake)
            ...
            if (i+1) % 2 == 0:
                out_cls = out_cls[:, :self.c_dim]  # fer2013
            else:
                out_cls = out_cls[:, :self.c_dim]  # ferg_db
            ...
            # ================== Train G ================== #
            if (i+1) % self.d_train_repeat == 0:
                # Original-to-target and target-to-original domain (fer2013)
                #fake_c = torch.cat([fake_c1, zero1, mask1], dim=1)
                #real_c = torch.cat([real_c1, zero1, mask1], dim=1)
                fake_c = torch.cat([fake_c1, mask1], dim=1)
                real_c = torch.cat([real_c1, mask1], dim=1)
                fake_x1 = self.G(real_x1, fake_c)
                rec_x1 = self.G(fake_x1, real_c)

                # Compute losses
                out, out_cls, _ = self.D(fake_x1)
                out_cls1 = out_cls[:, :self.c_dim]
                g_loss_fake = - torch.mean(out)
                g_loss_rec = torch.mean(torch.abs(real_x1 - rec_x1))
                g_loss_cls = F.cross_entropy(out_cls1, fake_label1)

                # Original-to-target and target-to-original domain (ferg_db)
                #fake_c = torch.cat([zero2, fake_c2, mask2], dim=1)
                #real_c = torch.cat([zero2, real_c2, mask2], dim=1)
                fake_c = torch.cat([fake_c2, mask2], dim=1)
                real_c = torch.cat([real_c2, mask2], dim=1)
                fake_x2 = self.G(real_x2, fake_c)
                rec_x2 = self.G(fake_x2, real_c)

                # Compute losses
                out, out_cls, _ = self.D(fake_x2)
    ...
    def test(self):
        ...
        if self.dataset == 'CelebA':
            ...
            data_loader = self.ferg_db_loader
    ...
    def test_multi(self):
        """Facial expression synthesis on fer2013 and ferg_db."""
        # Load trained parameters
        G_path = os.path.join(self.model_save_path, '{}_G.pth'.format(self.test_model))
        self.G.load_state_dict(torch.load(G_path))
        self.G.eval()

        for i, (real_x, org_c) in enumerate(self.fer2013_loader):
            ...
            for j in range(self.c_dim):
                target_c = self.one_hot(torch.ones(real_x.size(0)) * j, self.c_dim)
                target_c1_list.append(self.to_var(target_c, volatile=True))
            target_c2_list = []
            for j in range(self.c2_dim):
            ...
            #zero1 = self.to_var(torch.zeros(real_x.size(0), self.c2_dim))     # zero vector for fer2013 expressions
            mask1 = self.to_var(self.one_hot(torch.zeros(real_x.size(0)), 2)) # mask vector: [1, 0]
            #zero2 = self.to_var(torch.zeros(real_x.size(0), self.c_dim))      # zero vector for ferg_db expressions
            mask2 = self.to_var(self.one_hot(torch.ones(real_x.size(0)), 2))  # mask vector: [0, 1]

            # Changing emotional expressions on fer2013
            fake_image_list = [real_x]
            for j in range(self.c_dim):
                #target_c = torch.cat([target_c1_list[j], zero1, mask1], dim=1)
                target_c = torch.cat([target_c1_list[j], mask1], dim=1)
                fake_image_list.append(self.G(real_x, target_c))

            # Changing emotional expressions on ferg_db
            for j in range(self.c2_dim):
                #target_c = torch.cat([zero2, target_c2_list[j], mask2], dim=1)
                target_c = torch.cat([target_c2_list[j], mask2], dim=1)
                fake_image_list.append(self.G(real_x, target_c))
            fake_images = torch.cat(fake_image_list, dim=3)

            # Save the translated images
            save_path = os.path.join(self.result_path, 'fer2013_{}_fake.png'.format(i+1))
            save_image(self.denorm(fake_images.data), save_path, nrow=1, padding=0)
            print('Translated test images and saved into "{}"..!'.format(save_path))

        for i, (real_x, org_c) in enumerate(self.ferg_db_loader):
            ...
            # Save the translated images
            save_path = os.path.join(self.result_path, 'ferg_db_{}_fake.png'.format(i+1))
            save_image(self.denorm(fake_images.data), save_path, nrow=1, padding=0)
            print('Translated test images and saved into "{}"..!'.format(save_path))