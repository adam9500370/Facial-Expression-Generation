...
def main(config):
    ...
    if config.dataset in ['fer2013', 'BothFer']:
    ...
    if config.dataset in ['ferg_db', 'BothFer']:
        ferg_db_loader = get_loader(config.ferg_db_image_path, None, config.ferg_db_crop_size,
                                 config.image_size, config.batch_size, 'ferg_db', config.mode)

    # Solver
    solver = Solver(celebA_loader, rafd_loader, fer2013_loader, ferg_db_loader, config)

    if config.mode == 'train':
        if config.dataset in ['CelebA', 'RaFD', 'fer2013', 'ferg_db']:
            solver.train()
        elif config.dataset in ['Both', 'BothFer']:
            solver.train_multi()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fer2013_crop_size', type=int, default=48)
    parser.add_argument('--ferg_db_crop_size', type=int, default=256)
    ...
    parser.add_argument('--lambda_feat_rec', type=float, default=10)
    parser.add_argument('--d_train_repeat', type=int, default=5)

    # Training settings
    parser.add_argument('--dataset', type=str, default='CelebA', choices=['CelebA', 'RaFD', 'Both', 'fer2013', 'ferg_db', 'BothFer'])
    ...
    parser.add_argument('--fer2013_image_path', type=str, default='./data/fer2013/train')
    parser.add_argument('--ferg_db_image_path', type=str, default='./data/ferg_db/train')
